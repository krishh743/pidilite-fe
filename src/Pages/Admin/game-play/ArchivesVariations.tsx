...
const ArchivesVariations = ({ setWindow, window }) => {
  ...
  const fetchGamesList = async () => {
    try {
      const gamelistResponse = await fetch(
        `${baseUri}/api/variation?status=2`,
        {
          method: "GET",
          headers: {
            ...
            <tbody className="listTableBody">
              {gameListData?.map((game: gameOverview, index) => (
                <tr key={index}>
                  ...
                  <td className="viewColumn" style={{display:'flex',gap:10,justifyContent:"center",alignItems:"center",cursor:"pointer"}}>
                    <img
                      className="openEye"
...
export default ArchivesVariations;
